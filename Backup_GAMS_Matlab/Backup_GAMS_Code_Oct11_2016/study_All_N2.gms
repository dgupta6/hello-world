...
SETS
*DESIGN PARAMETERS
MH      Moving Horizon Length           /MH1*MH7/
RF      Rescheduling Frequency          /RF1*RF6/
...
comb(N,MH,RF,OPT)= no;
comb(N,MH,RF,OPT)$((ord(RF) eq 1 or ord(RF) eq 3 OR ord(RF) eq 6) AND ord(OPT) eq 1)= yes;
comb(N,MH,RF,OPT)$((ord(MH) eq 1 or ord(MH) eq 3 OR ord(MH) eq 5) AND ord(OPT) eq 1)= yes;
comb(N,MH,RF,OPT)$(ord(MH) eq 3 AND (ord(RF) eq 1 or ord(RF) eq 3 OR ord(RF) eq 6))= yes;

SET Allowed_MC(MC);
Allowed_MC(MC)=no;
Allowed_MC(MC)$(ord(MC) le 25)=yes;
