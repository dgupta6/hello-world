...
SETS
*DESIGN PARAMETERS
MH      Moving Horizon Length           /MH0*MH24/
RF      Rescheduling Frequency          /RF1*RF8/
...
*Testing space
*The demand here does not matter since what we are using is a custom demand in masterfile.gms
TABLE par_DF(N,DF)
        DF1     DF2     DF3
N2      3       6       12;
TABLE par_DV(DF,DV)
...
*par_MH(N,DF,MH)= 12+3*(ord(MH)-1)$(ord(MH) le 3)+6*(ord(MH)-2)$(ord(MH) ge 4);
par_MH(N,DF,MH)= 8+(ord(MH)-1);
*par_RF(N,DF,RF)= 3*(ord(RF)-1);par_RF(N,DF,RF)$(ord(RF) eq 1)= ord(RF);
par_RF(N,DF,RF)= ord(RF);
par_OPT(N,OPT)=0.025*(ord(OPT)-1);

*MH_max is important because it ensures that we consistently sample the same demands through MakeBatchFiles
...
Demand_combination(DF,DV,DL,DU,S)$(ord(DF) eq 1 AND ord(DL) eq 1 AND ord(DU) eq 1 AND ord(S) eq 1)=yes;
display Demand_combination;

SET N_Obj_combination(N,OBJ);
N_Obj_combination(N,OBJ)=no;
N_Obj_combination(N,OBJ)$(ord(OBJ) eq 1)=yes;
...
comb(N,MH,RF,OPT)= no;
comb(N,MH,RF,OPT)$(ord(OPT) eq 1)= yes;

SET Allowed_MC(MC);
Allowed_MC(MC)=no;
Allowed_MC(MC)$(ord(MC) le 1)=yes;
*Allowed_MC(MC)$(ord(MC) ge 26 AND ord(MC) le 50)=yes;
*Allowed_MC(MC)$(ord(MC) ge 51 AND ord(MC) le 75)=yes;
*Allowed_MC(MC)$(ord(MC) ge 76 AND ord(MC) le 100)=yes;
