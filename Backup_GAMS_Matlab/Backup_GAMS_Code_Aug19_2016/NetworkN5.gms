...
*         /(Reactor1).(TA,TB),Heater.Heating/
         ...
         /T1.U1,(T2,T3).U2,T4.U3/

         FIS(s) States with Finite Intermediate Storage /M1,M4/
         product_states(s) Product States/M2*M3/
         feed_states(s) Feed States/M0/
...
Parameters rho(i,s) proportion of input of task i from states s
           ...
           /T1.U1 20, (T2,T3).U2 10, T4.U3 20/
           vminij(i,j)  Minimum capacity of unit j when used for task i
           /T1.U1 10, (T2,T3).U2 5, T4.U3 1/
           Cs(s) maximum storage capacity for stage s  /(M0*M5) Inf/
           Cst(s) unit price of s/M0 1, M1 5, M2 10, M3 10, M4 100, M5 0/
;
*pis(i,s)=5;
pi(i)=smax(s,pis(i,s));
