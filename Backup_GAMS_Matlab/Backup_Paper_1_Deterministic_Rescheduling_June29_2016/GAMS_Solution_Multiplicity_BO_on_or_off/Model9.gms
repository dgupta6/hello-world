...
Ship_constraint(s,n)$((NOT Dst(s,n) gt 0) AND product_states(s)).. Ship_sales(s,n)=E=0;
...
record_relaxed_backlogTerminal(run,calc,s)
record_relaxed_inventory(run,calc,s,n)
record_relaxed_inventoryTerminal(run,calc,s)
record_relaxed_Rst(run,calc,s,n);
...
*$ontext
*$GDXIN GREEDY0_BO0_SHIP1_Z_N2_MH12_RF1_OPT1_DF3_DV1_DL3_DU1_S1_MC1_Profit.gdx
*$GDXIN GREEDY0_BO1_SHIP1_Z_N2_MH12_RF1_OPT1_DF3_DV1_DL3_DU1_S1_MC1_Profit.gdx
*$GDXIN BO0.gdx
$GDXIN BO1.gdx
$Load record_states record_inventory record_inventoryTerminal record_backlog record_backlogTerminal
$Load record_shipment record_shipment_sales record_Rst record_objective
$GDXIN
loop((run,calc)$(ord(calc)-1 eq 0),
Wbar.fx(i,j,n,hbar)=record_states(run,calc,'W',i,j,n,hbar);
Bbar.fx(i,j,n,hbar)=record_states(run,calc,'B',i,j,n,hbar);
*Sst.fx(s,n)=record_inventory(run,calc,s,n);
*Backlog.fx(s,n)=record_backlog(run,calc,s,n);
*obj_variable.fx=record_objective(run,calc);
SstTerminal.fx(s)=record_inventoryTerminal(run,calc,s);
BacklogTerminal.fx(s)=record_backlogTerminal(run,calc,s);
*Ship.fx(s,n)=record_shipment(run,calc,s,n);
*Ship_sales.fx(s,n)=record_shipment_sales(run,calc,s,n);
Rst.fx(s,n)=record_Rst(run,calc,s,n);
);
*$offtext
*display Wbar.l
...
record_relaxed_backlogTerminal(run,calc,s)=0;
record_relaxed_inventory(run,calc,s,n)=0;
record_relaxed_inventoryTerminal(run,calc,s)=0;
record_relaxed_Rst(run,calc,s,n)=0;
