...
if(%UnitBreakdown_Disturbance_flag% eq 1 and ord(calc)-1 ge 2,
         Breakdown(calc,i,j,hbar)$(Ij(i,j) and ord(hbar)-1 le pi(i) and Delay(calc-1,i,j,hbar-1) eq 0)=1$(uniform(0,1) gt 0.9 and Duration_breakdown(calc-1,j) eq 0 and (sum(n$(ord(n)-1 eq 0),record_states(run,calc-1,'W',i,j,n,hbar-1)) gt 0));

*If there is a delay ongoing then sample this breakdown
         Breakdown(calc,i,j,hbar)$(Ij(i,j) and ord(hbar)-1 le pi(i) and Delay(calc-1,i,j,hbar) gt 0)=1$(uniform(0,1) gt 0.9 and Duration_breakdown(calc-1,j) eq 0 and (sum(n$(ord(n)-1 eq 0),record_states(run,calc-1,'W',i,j,n,hbar)) gt 0));
...
*                                         +card(h)$(sum((hbar,i)$(Ij(i,j) and ord(hbar)-1 le pi(i)),Breakdown(calc,i,j,hbar)) eq 1);
         Duration_breakdown(calc,j)=2$(sum((hbar,i)$(Ij(i,j) and ord(hbar)-1 le pi(i)),Breakdown(calc-1,i,j,hbar)) eq 1 and Duration_breakdown(calc-1,j) eq (card(h)))
                                         +(Duration_breakdown(calc-1,j)-1)$(Duration_breakdown(calc-1,j) gt 0 and Duration_breakdown(calc-1,j) lt card(h))
                                         +(card(h))$(sum((hbar,i)$(Ij(i,j) and ord(hbar)-1 le pi(i)),Breakdown(calc,i,j,hbar)) eq 1);
...
*                                                            +2$(uniform(0,1) ge 0 and (sum(n$(ord(n)-1 eq 0),record_states(calc-1,'W',i,j,n,hbar-1)) gt 0) and Delay(calc-1,i,j,hbar-1) eq 0);
          Delay(calc,i,j,hbar)$(Ij(i,j) and ord(hbar)-1 le pi(i))=(Delay(calc-1,i,j,hbar)-1)$(Delay(calc-1,i,j,hbar) ge 1);
          Delay(calc,i,j,hbar)$(Ij(i,j) and ord(hbar)-1 le pi(i) and Delay(calc-1,i,j,hbar) eq 0)=2$(uniform(0,1) ge 0.6 and (sum(n$(ord(n)-1 eq 0),record_states(run,calc-1,'W',i,j,n,hbar-1)) gt 0 and Delay(calc-1,i,j,hbar-1) eq 0));
...
*BacklogTerminal.fx(s)=0;
$ontext
Backlog.fx(s,n)=Backlog.l(s,n);
BacklogTerminal.fx(s)=BacklogTerminal.l(s);
Wbar.fx(i,j,n,hbar)=Wbar.l(i,j,n,hbar);
Bbar.fx(i,j,n,hbar)=Bbar.l(i,j,n,hbar);
*Sst.fx(s,n)=round(Sst.l(s,n),2);
Sst.fx(s,n)=Sst.l(s,n);
Ship.fx(s,n)=Ship.l(s,n);
Ship_sales.fx(s,n)=Ship_sales.l(s,n);
Uut.fx(u,n)=Uut.l(u,n);
Rst.fx(s,n)=Rst.l(s,n);
SstTerminal.fx(s)=SstTerminal.l(s);
BacklogTerminal.fx(s)=BacklogTerminal.l(s);
NB.fx(i,j)=NB.l(i,j);
$offtext

*Backlog.fx(s,n)=round(Backlog.l(s,n),3);
*BacklogTerminal.fx(s)=round(BacklogTerminal.l(s),3);
Backlog.fx(s,n)=0;
BacklogTerminal.fx(s)=0;
Wbar.fx(i,j,n,hbar)=round(Wbar.l(i,j,n,hbar),3);
Bbar.fx(i,j,n,hbar)=round(Bbar.l(i,j,n,hbar),3);
*Sst.fx(s,n)=round(Sst.l(s,n),2);
Sst.fx(s,n)=round(Sst.l(s,n),3);
Ship.fx(s,n)=round(Ship.l(s,n),3);
Ship_sales.fx(s,n)=round(Ship_sales.l(s,n),3);
Uut.fx(u,n)=round(Uut.l(u,n),3);
Rst.fx(s,n)=round(Rst.l(s,n),3);
SstTerminal.fx(s)=round(SstTerminal.l(s),3);
BacklogTerminal.fx(s)=round(BacklogTerminal.l(s),3);
NB.fx(i,j)=round(NB.l(i,j),3);
Dst(s,n)=round(Dst(s,n),3);
rho(i,s)=round(rho(i,s),3);
rhobar(i,s)=round(rhobar(i,s),3);
