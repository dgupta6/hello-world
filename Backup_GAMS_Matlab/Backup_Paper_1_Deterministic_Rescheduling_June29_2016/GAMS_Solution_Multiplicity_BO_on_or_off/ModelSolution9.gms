...
Dst(s,n)$(product_states(s) AND ord(n)-1 eq 0) = 1;
*Wbar.fx(i,j,n,hbar)=0;
*Wbar.fx('T1','U1',n,'h0')$(ord(n)-1 eq 0 or ord(n)-1 eq 3)=1;
*Wbar.fx('T2','U2',n,'h0')$(ord(n)-1 eq 2 or ord(n)-1 eq 4)=1;
...
record_relaxed_backlogTerminal(run,calc,s)=BacklogTerminal.l(s);
record_relaxed_inventory(run,calc,s,n)=Sst.l(s,n);
record_relaxed_inventoryTerminal(run,calc,s)=SstTerminal.l(s);
record_relaxed_Rst(run,calc,s,n)=Rst.l(s,n);
);
***********************SOLVE RMIP TO GET INTEGRALITY GAP*****************
