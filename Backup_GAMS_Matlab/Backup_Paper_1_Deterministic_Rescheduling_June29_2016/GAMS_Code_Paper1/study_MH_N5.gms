...
SETS
*DESIGN PARAMETERS
MH      Moving Horizon Length           /MH1*MH8/
RF      Rescheduling Frequency          /RF1*RF4/
...
$offtext
par_DM(N,DF,DL)  /N5.DF1.(DL1 6,DL2 12,DL3 18)/;
par_DM(N,DF,DL)$(ord(N) eq 5)= par_DM(N,'DF1',DL)*(par_DF(N,DF)/par_DF(N,'DF1'));
...
par_MH(N,DF,MH)= 30+6*(ord(MH)-1);
par_RF(N,DF,RF)= 6*(ord(RF)-2);par_RF(N,DF,RF)$(ord(RF) eq 1)= 1;par_RF(N,DF,RF)$(ord(RF) eq 2)= 3;
...
comb_OBJ_DL(OBJ,DL)$(ord(OBJ) eq 1  AND ord(DL) eq 1)=yes;
*comb_OBJ_DL(OBJ,DL)$(ord(OBJ) eq 2 AND ord(DL) eq 1)=yes;


SET comb(N,MH,RF,OPT);
comb(N,MH,RF,OPT)= no;
comb(N,MH,RF,OPT)$(ord(N) eq 5)= yes;
