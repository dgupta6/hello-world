...
awk '{print "condor_rm "$2}' condor_job_list_raw > rm_condor_job_list.bat
chmod +x rm_condor_job_list.bat
...
then
	...
	if test "$n_runs" -lt "10";
	 ...
	 echo "list: see list, rm:remove from cluster, y: resubmit to cluster, yy:run sequentially, n: don't do anything"
	 echo "re-run unsuccessful jobs? (list,rm,y,yy or n)"
         read -e answer
         if test "$answer" == "yy";
            then
                 ./IncompleteRuns.sh
