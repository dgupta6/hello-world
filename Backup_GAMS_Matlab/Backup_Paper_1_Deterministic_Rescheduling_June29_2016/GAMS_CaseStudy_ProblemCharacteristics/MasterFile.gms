...
*Dst_full_realization('M3',h,run)=1.5*Dst_full_realization('M3',h,run);
*Dst_full_realization(s,h,run)$(ord(h)-1 le 7)=0; 
*);
********************DEMAND SAMPLING TO KEEP IT STANDARDIZED FOR ALL APPROACHES(NOMINAL, ROBUST, STOCHASTIC)************************
...
*Write OPEN loop static gantt chart %GanttChart_OPENLOOP% eq 1
         loop((run,calc)$(ord(run) eq 1 and ord(calc) eq %Gantt_OL_Number%),
