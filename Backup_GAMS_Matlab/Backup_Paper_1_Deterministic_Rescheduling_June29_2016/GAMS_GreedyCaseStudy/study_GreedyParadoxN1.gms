...
RF      Rescheduling Frequency          /RF1*RF2/
OPT     Optimality gap                  /OPT1*OPT11/
...
PARAMETER
par_DM(N,DF,DL)  /N1.DF1.(DL1 3,DL2 4,DL3 5)/;
par_DM(N,DF,DL)$(ord(N) eq 1)= par_DM(N,'DF1',DL)*(par_DF(N,DF)/par_DF(N,'DF1'));
...
par_RF(N,DF,RF)$(ord(RF) eq 1)= 1;par_RF(N,DF,RF)$(ord(RF) eq 2)= 3;
...
N_DL_combination(N,DL)$(ord(N) eq 1)=yes;
...
N_MH_combination(N,MH)$(ORD(N) eq 1)=yes;
