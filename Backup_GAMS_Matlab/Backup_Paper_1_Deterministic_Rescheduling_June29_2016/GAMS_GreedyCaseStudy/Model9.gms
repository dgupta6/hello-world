...
objective_MAX_PROFIT_Greedy.. obj_variable =E= sum((s,n)$(product_states(s)),round(2-(ord(n)/card(n)),2)*Cst_new(s)*(Ship_sales(s,n)))
                                         -sum((i,j,n,hbar)$(Ij(i,j) AND ord(hbar)-1 eq 0),Wbar(i,j,n,hbar))
                                         -1E2*sum((s,n)$(product_states(s)),Cst_new(s)*Backlog(s,n))
                                         -1E2*sum(s$(product_states(s)),Cst_new(s)*BacklogTerminal(s));
...
NoShip(s,n)$(ord(n) lt floor(card(n)/2) and sameas(s,'M4')).. Ship_sales(s,n)=E=0;
