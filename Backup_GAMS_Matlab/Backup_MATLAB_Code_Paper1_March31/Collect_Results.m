...
% N=2;
N=3;
for plot_MH_RF_OPT_Greedy = 2
    for plot_cost_profit = 1
            ...
            for dl = 1:length(DL{df})
                ...
                if flag_plot_MH == 1
                    if (scale_data==1)
                        ...
                        %                         data_MC
                        for mc=1:length(MC{df})
                            data_MC(:,:,mc) = data_MC(:,:,mc)/data_MC(1,end,mc); %Scale (Fastest RF, Longest MH)
                        end
                    end
                    data = mean(data_MC,3); %Average
                    std_dev = std(data_MC,0,3); %standard deviation
                    ...
                    save([pwd,'\DATA\MH\MH_',Obj,...
                        '_DF_',num2str(DF{df}),'_DL_',num2str(DL{df}(dl))],'save_data');
                ...
                elseif flag_plot_RF == 1
                    ...
                    if (scale_data==1)
                        %std_dev = std_dev/data(1,end);
                        for mc=1:length(MC{df})
                            data_MC(:,:,mc) = data_MC(:,:,mc)/data_MC(end,1,mc); %Scale (Longest MH, Fastest RF)
                        end
                    end
                    data = mean(data_MC,3); %Average
                    std_dev = std(data_MC,0,3); %standard deviation
                ...
                elseif flag_plot_OPT == 1
                    ...
                    if (scale_data==1)
                        ...
                        %                         data_MC
                        for mc=1:length(MC{df})
                            data_MC(:,:,mc) = data_MC(:,:,mc)/data_MC(1,1,mc); %Scale (Fastest RF, Best OPT)
                        end
                    end
                    data = mean(data_MC,3); %Average
                    std_dev = std(data_MC,0,3); %standard deviation
                    ...
                    for rf=1:length(RF{df})
                        IntGap_data(rf) = mean2(IntegralityGap_data_MC(1:RF{df}(rf):(n_calc+1),rf,:,:));
