...
corr_flag=3; %Pearson, Kendall, Spearman
...
%     for plot_MH_RF_OPT_Greedy = [4]
    if plot_MH_RF_OPT_Greedy <= 3
        ...
        table = zeros(N_factors_each_subplot*dl_upper,9);
        
        for df=1:3
            for dl=1:dl_upper
                for index=1:N_factors_each_subplot
                    table(index + N_factors_each_subplot*(dl-1),3*(df-1)+1)=p_value{plot_MH_RF_OPT_Greedy,plot_cost_profit,df,dl,index}(1);
                    table(index + N_factors_each_subplot*(dl-1),3*(df-1)+2)=corr_value{corr_flag,plot_MH_RF_OPT_Greedy,plot_cost_profit,df,dl,index}(1);
                    table(index + N_factors_each_subplot*(dl-1),3*(df-1)+3)=corr_p_value{corr_flag,plot_MH_RF_OPT_Greedy,plot_cost_profit,df,dl,index}(1);
