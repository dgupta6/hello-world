...
for df=1:length(DF)
    for dl=1:length(DL{df})
        load([pwd,'\DATA\OPT\N',num2str(N),'_OPT_',Obj,'_DF',num2str(DF{df}),...
            '_DL',num2str(DL{df}(dl))],'save_data');
        append_data=[append_data;save_data(:,2:end)'];
    end
end
for i=1:size(append_data,1)
    append_data(i,:)=append_data(i,:)/append_data(i,1);
end
...
set(gca,'XTick',OPT,'YTick',[0:2.5:10],'YMinorTick','on','FontSize',12);
grid on
box on
...
ylabel('Gap (%)')
legend({'Est Gap','True Gap','CL Gap'},'Location','NorthWest')
set(gca,'XTick',OPT,'YTick',[0:5],'YMinorTick','on','FontSize',12);
grid on
box on
